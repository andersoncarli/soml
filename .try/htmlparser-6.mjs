...
function html2soml(htmlFragment, options = { format: 'object' }) {
  const stack = [{}];
  const elementCounts = {};
  ...
  function createKey(name, attributes) {
    ...
    if (attributes.id) {
      key += `#${attributes.id}`;
    } else if (elementCounts[key]) {
      // If no id provided and it's not the first element of its kind, generate an id
      elementCounts[key]++;
      ...
      ontext(text) {
        ...
        if (trimmedText) {
          ...
          if (Object.keys(parent).length === 0) {
            ...
            const parentKey = Object.keys(stack[stack.length - 2])[0];
            if (options.format === 'array') {
              stack[stack.length - 2][parentKey][stack[stack.length - 2][parentKey].length - 1] = trimmedText;
            } else {
              stack[stack.length - 2][parentKey] = trimmedText;
            }
          } else {
...
const htmlFragment = `
  <div style="color: red; font-size: 14px;">
    ...
    <p class="note red">Another paragraph</p>
    <p>Third paragraph</p>
  </div>
`;
...
console.log(JSON.stringify(html2soml(htmlFragment, { format: 'array' }), null, 2));